...
class BaseView(View):
    ...
    for i in views['cat_details']:
        ...
        a = {'cats': i.title, 'image': i.image, 'cat_count': count_item, 'url': i.get_cat_url}
    ...
    if request.method == 'POST':
        ...
        if len(name) < 3 or len(message) < 5 or len(email) < 4:
            ...
            send_email = EmailMessage(
                ...
                f'Hello admin, {name} is trying to contact you. His email is {email}. He want to talk about {subject}.His message is {message}.',
...
def cart(request, slug):
    ...
    if Cart.objects.filter(slug=slug).exists():
        ...
        data.save()
        data1.save()
        messages.success(request, '✔️The item is added in cart!')
...
#             )
#             data.save()
#             messages.info(request, 'The cart is checked out!')
